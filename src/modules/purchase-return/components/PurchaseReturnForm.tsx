...
const purchaseReturnSchema = z.object({
  purchaseId: requiredSelect("Purchase is required"),
  items: z
    .array(
      z.object({
...
export const PurchaseReturnForm = ({ onSubmit, isLoading }: PurchaseReturnFormProps) => {
  ...
  const getPurchaseLabel = (p: (typeof purchases)[0]) => {
    const supplier = typeof p.supplierId === "object" ? p.supplierId.name : p.supplierId;
    return `${p.invoiceNumber} — ${supplier}`;
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Button type="submit" className="w-full" disabled={isLoading}>
        {isLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating...</>) : "Create Purchase Return"}
      </Button>
    </form>
  );
};
